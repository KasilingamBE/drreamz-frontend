...
const DriverInbox = ({userData,isSpaceOwner}) => {
    ...
    useEffect(()=>{
        ...
        client.query({query:GET_DRIVER_MESSAGES,variables:{driverId:userData.sub}}).then(({data})=>{
    ...
    return (
        ...
        <div className="dg__account">
            ...
            <div className="inbox-list">
                {messages.length>0? messages.map((item)=><InboxListItem key={item[0].listingId} listingId={item[0].listingId} listingAddress={item[0].listingAddress} lastMessage={item[item.length-1]}  />):<div className="loading">No messages found</div>}            
            </div>
}
        </div>
        </DriverContainer>
