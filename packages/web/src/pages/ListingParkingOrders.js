...
const GET_BOOKINGS_WITH_LISTINGID_AND_STATUS = gql`
  query GetBookingsWithListingIdAndStatus($listingId: ID!, $status: String!) {
    getBookingsWithListingIdAndStatus(listingId: $listingId, status: $status) {
      _id
      driverId
      driverName
      driverEmail
...
const ListingParkingOrders = ({ id }) => {
  ...
  const updateBookingVehicle = (id, data) => {
    setUpcoming(
      upcoming.map((item) =>
        item._id === id ? { ...item, vehicle: data } : item
      )
    );
  };
  ...
  const updateBookingHandler = async (data) => {
    try {
      ...
      setUpcoming(
        upcoming.map((item) =>
          item._id === data.id ? { ...item, ...data, _id: data.id } : item
        )
      );
  ...
  const earlyCheckInHandler = async (data) => {
    try {
      ...
      setUpcoming(
        upcoming.map((item) =>
          item._id === data.id ? { ...item, start: data.start } : item
        )
      );
    } catch (error) {
  ...
  useEffect(() => {
    client
      .query({
        query: GET_BOOKINGS_WITH_LISTINGID_AND_STATUS,
        variables: { listingId: id, status: "upcoming" },
      })
      .then(({ data }) => {
        if (data.getBookingsWithListingIdAndStatus) {
          console.log(data.getBookingsWithListingIdAndStatus);
          setUpcoming(data.getBookingsWithListingIdAndStatus);
        }
      })
      .catch((error) => {
        console.log(error);
    ...
    client
      .query({
        query: GET_BOOKINGS_WITH_LISTINGID_AND_STATUS,
        variables: { listingId: id, status: "current" },
      })
      .then(({ data }) => {
        if (data.getBookingsWithListingIdAndStatus) {
          console.log(data.getBookingsWithListingIdAndStatus);
          setCurrent(data.getBookingsWithListingIdAndStatus);
        }
      })
      .catch((error) => {
        console.log(error);
