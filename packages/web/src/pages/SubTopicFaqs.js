...
const UPDATE_FAQ = gql`
  mutation UpdateFaq($id:ID!,$roles:[String],$topic:String,$question:String,$answer:String){
...
const SubTopicFaqs = ({topic,authRole}) => {
    ...
    useEffect(()=>{
        setLoading(true);
        client.query({query:GET_FAQS_BY_TOPIC,variables:{topic:topic}}).then(({data})=>{
    ...
    return (
        <div className="dg__account" >
            <h1 className="heading">Frequently Asked Questions
</h1>
<p className="lead">Topic : {topic}</p>
        {loading? <div className="loading">Loading...</div>:
            faqs.length>0 ?<Accordion> {faqs.map((item) => 
                <Card className="faq-item">
                  <Card.Header>
                      {item.question}
                      <div class="faq-ctrl-btns">
                          {authRole==="admin" && <>
                          <Button variant="primary" onClick={()=>{editButtonHandler(item)}}><MdEdit/></Button>
                         {editFaq && <AddFaqModal show={faqModal} edit={true} prevData={editFaq} handleClose={()=>{setFaqModal(false)}} handleSave={()=>{}} handleUpdate={updateFaqHandler}/>}
                          <Button variant="danger" onClick={()=>{deleteFaqHandler(item._id)}}><MdDelete/></Button>
                          </>}
                ...
                )}</Accordion>:<div className="loading">No FAQs Found!</div>}
