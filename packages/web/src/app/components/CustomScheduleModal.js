...
const CustomScheduleModal = ({ show, handleClose,handleSave }) => {
  ...
  if(document.querySelector('.single-start-time-input')){
    document.querySelector('.single-start-time-input').addEventListener('mouseleave', function() {
      ...
      document.querySelector(".single-start-time-input").value="";
        ...
        document.querySelector(".single-start-time-input").value = moment(selectedDate[0]).format('LT');
        }
      };
    });
  }
  ...
  if(document.querySelector('.multiple-start-time-input')){
    document.querySelector('.multiple-start-time-input').addEventListener('mouseleave', function() {
      if(document.querySelector(".multiple-start-time-input").value==""){
        if(moment(multipleDates[0][0]).format('LT')==="Invalid date"){
          document.querySelector(".multiple-start-time-input").value="";
        }else{
        document.querySelector(".multiple-start-time-input").value = moment(multipleDates[0][0]).format('LT');
        }
      };
    });
  }
  ...
  const handleMultipleDateSelect = (day, { selected }) => {
    ...
    if (selected) {
      ...
      if(selectedIndex.length>0){
        setMultipleDates(multipleDates.filter(selectedDay =>
          !DateUtils.isSameDay(selectedDay[1], day)
        ));
      }
    } else {
...
useEffect(()=>{
  ...
  return (
    <Modal show={show} onHide={handleClose}>
      ...
      <Nav variant="pills" activeKey={activeType} style={{display:'flex',justifyContent:'center'}}>
  <Nav.Item>
    <Nav.Link eventKey="single" onClick={()=>{setActiveType('single')}}>Single</Nav.Link>
  </Nav.Item>
  <Nav.Item>
    <Nav.Link eventKey="multiple" onClick={()=>{setActiveType('multiple')}}>Multiple</Nav.Link>
  </Nav.Item>
  <Nav.Item>
    <Nav.Link eventKey="range" onClick={()=>{setActiveType('range')}}>
      Range
    </Nav.Link>
  </Nav.Item>
</Nav>
        ...
        <DayPicker
          ...
          onDayClick={handleSingleDateSelect}
        />
        {selectedDate[0] && 
<Form>
            <Form.Group controlId='exampleForm.SelectCustom' style={{marginTop:'5px !important'}} >
              <Form.Label>Start Time</Form.Label>
              <input list="single-start-time" value={moment(selectedDate[0]).format('LT')==="Invalid date"?"":moment(selectedDate[0]).format('LT')} className="form-control single-start-time-input" placeholder="Start Time" onChange={(event)=> {console.log("start time :",moment(event.target.value,"LT").isValid(),selectedDate[0],event.target.value,moment(`${moment(selectedDate[0]).format('ll')} ${event.target.value}`,'lll')._d); if(moment(event.target.value,"LT").isValid()){  setSelectedDate([moment(`${moment(selectedDate[0]).format('ll')} ${event.target.value}`,'lll')._d,moment(`${moment(selectedDate[0]).format('ll')} ${event.target.value}`,'lll').add(1,'hour')._d])}}} />
            ...
            <Form.Group controlId='exampleForm.SelectCustom mt-2' style={{marginTop:'5px !important'}} >
              <Form.Label>End Time</Form.Label>
              
              <input list="single-end-time" value={moment(selectedDate[1]).format('LT')==="Invalid date"?"":moment(selectedDate[1]).format('LT')} className="form-control single-end-time-input" placeholder="End Time" onChange={(event)=> {setSelectedDate([selectedDate[0],moment(`${moment(selectedDate[1]).format('ll')} ${event.target.value}`,'lll')._d])}}/>
        ...
        {activeType=="multiple" && <> <DayPicker
          selectedDays={[].concat(...multipleDates)}
          onDayClick={handleMultipleDateSelect}
        />
        {multipleDates.length>0 && <Form>
          <Form.Group controlId='exampleForm.SelectCustom' style={{marginTop:'5px !important'}} >
            <Form.Label>Start Time</Form.Label>
            <input list="multiple-start-time" value={moment(multipleDates[0][0]).format('LT')==="Invalid date"?"":moment(multipleDates[0][0]).format('LT')} className="form-control multiple-start-time-input" placeholder="Start Time" onChange={(event)=> {setMultipleDates(multipleDates.map((item)=>[moment(`${moment(item[0]).format('ll')} ${event.target.value}`,'lll')._d,item[1]]))}} />
          ...
          <Form.Group controlId='exampleForm.SelectCustom mt-2' style={{marginTop:'5px !important'}} >
            ...
            <input list="multiple-end-time" value={moment(multipleDates[0][1]).format('LT')==="Invalid date"?"":moment(multipleDates[0][1]).format('LT')} className="form-control multiple-end-time-input" placeholder="End Time" onChange={(event)=> {setMultipleDates(multipleDates.map((item,index)=>[item[0],moment(`${moment(item[1]).format('ll')} ${event.target.value}`,'lll')._d]))}}/>
        ...
        {from && to && <Form>
          <Form.Group controlId='exampleForm.SelectCustom' style={{marginTop:'5px !important'}} >
            <Form.Label>Start Time</Form.Label>
            <input list="range-start-time" value={moment(from).format('LT')==="Invalid date"?"":moment(from).format('LT')} className="form-control range-start-time-input" placeholder="Start Time" onChange={(event)=> {console.log(event.target.value,from); setRange({from:moment(`${moment(from).format('ll')} ${event.target.value}`,'lll')._d,to:moment(`${moment(to).format('ll')} ${event.target.value}`,'lll')._d})}} />
          ...
          <Form.Group controlId='exampleForm.SelectCustom mt-2' style={{marginTop:'5px !important'}} >
            ...
            <input list="range-end-time" value={moment(to).format('LT')==="Invalid date"?"":moment(to).format('LT')} className="form-control range-end-time-input" placeholder="End Time" onChange={(event)=> {setRange({...range,to:moment(`${moment(to).format('ll')} ${event.target.value}`)._d})}}/>
