...
const PublishListingModal = ({show,handleClose,id,updateListingLocal,userId}) => {
  ...
  setStartTimeArr(timeArray);
    ...
    const checkWithdrawalSettings = async ()=>{
        ...
        let data = await client.query({query:stripe_Retrieve_Account,variables:{userId:userId}});
        // .then(({data})=>{
    ...
    return (
        <Modal show={show} onHide={handleClose} className="faq-modal" backdrop="static" keyboard={false}>
      <Modal.Header closeButton>
        <Modal.Title>Activate Your Listing</Modal.Title>
      </Modal.Header>
      <Modal.Body>
            <p className="lead">Your listing has been successfully created. Do you want to go live now?</p>
            <Button variant="primary" disabled={disabled} onClick={handlePublish}>Activate</Button>
            <br/>
            <br/>
            <p className="lead">Tell us when do you want to go live.</p>
            <div className='timepicker' style={{display:'flex',alignItems:'flex-start'}}>
                <div>
          <p className='description' style={{marginTop: '0px',color:'#000'}}>Start Date</p>
          <DatePicker clearIcon={null} value={time} minDate={new Date()} onChange={(value)=>{setTime(moment(`${moment(value).format('ll')} ${moment(time).format('LT')}`,'lll')._d) }} />
          </div>
          <Form>
            <Form.Group controlId='exampleForm.SelectCustom' style={{marginTop:'5px !important'}} >
              <Form.Label>Start Time</Form.Label>
              <input list="start-time" value={moment(time).format('LT')} className="form-control start-time-input" placeholder="Start Time" onChange={(event)=> {setTime(moment(`${moment(time).format('ll')} ${event.target.value}`,'lll')._d);}} />
          ...
          <Modal.Footer>
                <Button variant="secondary" onClick={handleClose} disabled={disabled}>Close</Button>
                <Button variant="primary" onClick={()=>{handleClose()}} disabled={disabled}>Save</Button>
          </Modal.Footer>
          </Modal>
    )
}
