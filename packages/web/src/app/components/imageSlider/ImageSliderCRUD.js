...
function ImageSliderCRUD(props) {
  ...
  const [publishImageSliderMutation] = useMutation(
    PUBLISH_UPDATE_ONE_IMAGE_SLIDER
  );
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      ...
      setAllRooms(
        allRooms.map((r) =>
          r._id === payload._id ? res.data.updateOneImageSlider : r
        )
      );
      props.dispatch(hideLoading());
      updateDisabled(false);
